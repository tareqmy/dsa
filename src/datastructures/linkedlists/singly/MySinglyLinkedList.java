...
public class MySinglyLinkedList {
    ...
    //is it better? certainly more readable.
    public void reverse2() {
        if(length == 1) {
            return;
        }
        MySinglyLinkedNode current = this.head;
        MySinglyLinkedList newList = new MySinglyLinkedList(current.getValue()); //because in this implementation one entry is must

        while (current.getNext() != null) {
            newList.prepend(current.getNext().getValue());
...
//        mySinglyLinkedList.remove(2);
        mySinglyLinkedList.printList();
        mySinglyLinkedList.reverse2();
        mySinglyLinkedList.printList();
        System.out.println(mySinglyLinkedList.getHead().getValue());
        System.out.println(mySinglyLinkedList.getTail().getValue());
